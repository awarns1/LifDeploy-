...
public class ConfigPayDunyaImpl implements IConfigPayDunya {
	...
	public String baseConfig(Donation donation, HttpServletRequest request) {
		...
		setup.setPublicKey("live_public_MrtsfBLEwm2qs1sTytkiKkrSBiZ"); // free key :  test_public_frnXPQJVbb0WVsbrDdWnbJrKbX5
		...
		store.setCancelUrl(appUrl + "/donation");
		...
		invoice.setTotalAmount(donation.getMontant());
		invoice.setDescription("Montant donation: " + donation.getMontant() + " FCFA");
		invoice.create();
		if (invoice.create()) {
			donation.setToken(invoice.getToken());
